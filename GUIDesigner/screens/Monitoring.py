...
class Monitoring(ScreenBase):
    ...
    def _create_widgets(self):
        ...
        stm_plus_buttons: List[tk.Button] = []
        stm_search_buttons: List[tk.Button] = []
        stm_minus_buttons: List[tk.Button] = []
        ...
        # conv_g_commands = ["CONV 0,GOOD"]
        # conv_b_commands = ["CONV 0,BAD"]
        ...
        def enable_stm_button(flag):
            if flag == 0:  # 原点サーチ
                ...
                stm_reset_button["state"] = "disabled"
        ...
        stm_reset_button = tk.Button(
            self, text="リセット", state="disabled", width=10, bg="#87de87", font=("MSゴシック", BUTTON_FONT_SIZE, "bold"))
        ...
        stm_turn_button["command"] = lambda: (self.robot_oprration_request("STM 0,R,"+str(self.stm_motor_turn)),
                                              ...
                                              enable_stm_button(0))
        stm_reset_button["command"] = lambda: (enable_stm_button(1))
        ...
        stm_reset_button.grid(row=11, column=3)
